...
describe('RegistrationModel', () => {
    ...
    beforeAll(async () => {
        ...
        const user = await UserModel.create({ name: 'Reg User', email: 'reguser@example.com', password: 'pw' });
        userId = user.id;
    ...
    it('should delete registration by id', async () => {
        ...
        // Should be gone
        const regs = await prisma.registration.findMany({ where: { id: registrationId } });
    ...
    it('should create and delete by composite key', async () => {
        ...
        const regs = await prisma.registration.findMany({ where: { eventId, userId } });
        expect(regs.length).toBe(0);
    });
});
